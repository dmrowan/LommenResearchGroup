...
def main():
    ...
    if (interpulse == "y"):
        popt2, pcov2 = curve_fit(gauss, xvals, yvals, bounds = ([min(yvals)-100, 0.2, 0.025, min(yvals)-100], [max(yvals), 0.4, 0.1, max(yvals)]))
        plt.plot(xvals, gauss(xvals, *popt2), color = 'orange') # plots the fitted curve to interpulse
    plt.plot(xvals, gauss(xvals, *popt), color = 'orange')  # plots the fitted curve to left side of pulse

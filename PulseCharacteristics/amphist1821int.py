...
def main():    
    ...
    for n in range(len(phases)):
        ...
        popt3, pcov3 = curve_fit(gauss, xvals, yvals, bounds = ([min(yvals)-100, 0.2, 0.025, min(yvals)-100], [max(yvals), 0.4, 0.1, max(yvals)])) # uses gaussian function to do a curve fit to the line version fo the histogram 
       # plt.plot(xvals, gauss(xvals, *popt))

        # Amplitude of fitted curve
        amp = popt3[0]  # finds amplitude of fitted curve (the first parameter of curve fit)

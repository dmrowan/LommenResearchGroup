...
def fit(pulsarname, timewidth):
    ...
    if (pulsarname == '1937'):
        intint = pd.read_csv('intdata2/1937intdata_%s.txt' %timewidth, header = None)
    if (pulsarname == '1821'):
        intint = pd.read_csv('intdata2/1821intdata_%s.txt' %timewidth, header = None)
    ...
    plt.xlabel('Integrated Intensity (counts/second)')
    plt.ylabel('Counts')
    plt.title('Integrated Intensities of Pulse Profiles')
    if (pulsarname == '1937'):
        plt.savefig('intplots2/1937_%s.png' % timewidth)
    if (pulsarname == '1821'):
        plt.savefig('intplots2/1821_%s.png' % timewidth)
    plt.clf()
...
errorbars = []
timewidth=[]
for twidth in range(2700, 9000, 900): 
    #if (twidth == 1800):
    #    twidth = 2100

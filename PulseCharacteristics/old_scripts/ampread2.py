...
def intint(n_rotations):
    ...
    for name in filenames:
        ...
        try: 
            peakloc, standdev, intloc, intstanddev = findpeak(phase, showplot=True)
        ...
        for n in range(number):
            ...
            try:
                ...
                if saveplot == True:
                    ...
                    #plt.savefig('profile%s.png'%name)
                    plt.show()
                    plt.clf()
                    saveplot = False
                """
            ...
            f = open("intdata/%sintdata_%s.txt" %(psr, n_rotations), "a")
...
if psr == 1821:
    freq = 327.4
rotations = [2700, 3600, 4500, 5400, 6300, 7200, 8100] #rotations
for i in range(len(rotations)):
    rotations[i] = int(rotations[i]*freq)

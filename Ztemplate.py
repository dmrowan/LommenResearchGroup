...
desc="""
Read an FT1 file containing PULSE_PHASE info and interactively fit a template.
"""

def light_curve(phases, weights=None, nbins=25, ec='blue', ls='solid',
                label=None,axes=None,fignum=1,nmc=100,template=None):
...
def get_phases(ft1file, get_weights=False, 
               weightcol='WEIGHT', tmax=999999999):
...
class BiasedFitter(object):
    ...
    def welcome(self):
        print('\nWelcome ( ͡° ͜ʖ ͡°) . This is an updated non-interactive'\
              'version of nitemplate.py the NICERsoft likelihood'\
              'estimation script.')
    ...
    def write_profile(self, outfile, nbin, integral=True, suppress_bg=True):
        if not hasattr(self,'fitter'):
            print('Must do fit first!') 
            return
        self.fitter.template.write_profile(outfile, nbin, 
                                           integral=integral,
...
if __name__ == '__main__':
    ...
    parser.add_option('--nhistbins',type='int',default=50,
                      help="Number of bins to use in phase histogram.")
    parser.add_option('--nprofbins',type='int',default=256,
                      help="Number of bins to use output tabular profile.")
    parser.add_option('-u','--unbinned',action='store_true',default=False,
                      help="Perform fit with unbinned likelihood.")
    parser.add_option('-a','--align',action='store_true',default=False,
                      help="Align template such that peak falls at phase 0.")
    parser.add_option('-w','--weights',action='store_true',default=False,
                      help='Use weighted light curve')
    parser.add_option('-c','--weightcol',type='string',default='WEIGHT',
                      help='Column in FITS file that holds the weight')
    parser.add_option('-p','--prof',type='string',default=None,
                      help='Output name for products (default itemplate.*')
    parser.add_option('-m','--min_weight',type='float',default=1e-2,
                      help='Minimum weight to include in fit.')
    parser.add_option('-T','--tmax',type='float',default=999999999,
                      help='Maximum time to include in fit.')
    parser.add_option('-e','--errors',action='store_true',default=False,
                      help='Compute errors on components.')
    parser.add_option('-P','--pulsar',action='store',
                      default = 'unidentified pulsar',
                      type='string',dest='pulsar_name')
    ...
    if False:
        ...
        #main()
        intf = BiasedFitter(phases, nbins=options.nhistbins, weights=weights,
                            errors=options.errors,unbinned=options.unbinned)
        #pl.close()
        ...
        if options.prof is not None:
            ...
            if len(dname) > 0 and not os.path.exists(dname):
                raise IOError('Specified directory %s does not exist!'%(
                    os.path.dirname(out)))
            prof = options.prof
        else:
...
╭( ✖_✖ )╮If the fitting failed, please re-execute the script without 
using the < -P > flag in order to input the right initial estimates. 
\n\n\nIf the fitting was successful, fetch phase/ampl/fwhm data by 
calling < cat itemplate.gauss>
\n\nGoodbye ~~
""")

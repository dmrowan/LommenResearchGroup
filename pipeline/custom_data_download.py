...
desc = """
This is a modified version of the nicersoft script ni_data_download.py

Only small tweaks have been made for better integration with the Haverford pipeline system. 

Changes:
    - option to download specific obsids with --obsIDs flag
    - curl command using subprocess
    - checks both basedir paths
...
parser.add_argument("--outdir",     help="Directory to place all ObsIDs (relative to current)", type=str, default=None)
parser.add_argument("--clobber",    help="Clobber existing files", action='store_true')
parser.add_argument("--cl-only",    help="Grab the clean event files only", action='store_true', dest='cl_only')
...
for n,[no, row] in enumerate(source.iterrows()):
         ...
         #Base is public archive
         base = "ftp://heasarc.gsfc.nasa.gov/nicer/data/obs/"

         #Alternative path to team archive
         base_team = "ftp://heasarc.gsfc.nasa.gov/nicer/data/.obs2validate/"  

         # Select the date folder
         base += "{}_{:02d}/".format(dtime.year, dtime.month)
         base_team += "{}_{:02d}/".format(dtime.year, dtime.month)
         ...
         if not matchedfiles or args.clobber:
             ...
             log.info("{:3d} / {:3d} :: Obsid {} has already been downloaded".format(n+1, len(source), obsid))
...
if (args.decryptkey and not decryptOK) or not args.decryptkey:
    log.warning('Data files not decrypted. The following command can decrypt all files:')
    print(" find {} -name '*.gpg' -exec gpg --batch --yes --passphrase 'your_passphrase'  {{}} \; -delete".format(workingdir))
    
if (args.unzip and not decryptOK) or not args.unzip:
    log.warning('Data files not unzipped. The following command can unzip all (decrypted) files:')

...
def processdata(obsID, par):
    ...
    hdulist = fits.open('%s/intermediate.evt'%pipedir)
    n_events = hdulist[1].header['NAXIS2']
    log.info('There are %s events for this ObsID'%n_events)
    if n_events == 0:
        raise ValueError("Zero valid events for this ObsID")

    cmd = ["/homes/alevina/PINT/src/pint/scripts/photonphase.py", '--polycos', '--addphase', '%s/cleanfilt2.evt'%pipedir, par]
    runcmd(cmd)
